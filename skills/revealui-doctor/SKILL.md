...
- `~/revealfleet/revealui/.claude/rules/`
...
- Extract referenced script paths under `~/.claude`, `~/revealfleet/revskills`, `node`/`bash` invocations.
- Assert each referenced script exists. Report missing.
- Flag any `~/suite/` references as stale (retired 2026-05-07 → `~/revealfleet/`).

## 4. Git integrity (RevFleet repos)

For each repo in `~/revealfleet/revealui` and `$JV_REPO`:
```bash
...
`~/revealfleet/revealui/.claude/events.jsonl` and `$JV_REPO/.claude/events.jsonl`. Warn if over 100KB.
...
cd ~/revealfleet/revealui && test -f flake.lock && nix flake metadata --json >/dev/null 2>&1 && echo "flake: ok" || echo "flake: check"
cd ~/revealfleet/revealui && direnv status 2>&1 | tail -3
```

## 12. Disaster recovery (WSL snapshot)

Per-repo LTS sync is retired. Flag leftover `.claude/lts-mode` under `~/revealfleet/*/`. Snapshot freshness is Windows-side — SKIP with pointer to revkit `check-backup-staleness.ps1` when LTS drive unreachable.
