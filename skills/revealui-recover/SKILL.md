...
```bash
node "$REVEALFLEET_ROOT/revskills/scripts/recover-inventory.js" --hours 72
```
...
- Also report dirty shared checkouts and `$REVEALFLEET_ROOT/.wt/*` that are not clean. Do not commit a dirty shared `.jv` checkout that a peer owns.
...
```bash
grep -l -r "$(basename "$REPO")" "$HOME"/.claude/projects/*revealfleet*/memory/ "$HOME"/.claude/projects/*revealfleet*/memory/ 2>/dev/null
```
...
```bash
node "$REVEALFLEET_ROOT/revskills/scripts/recover-inventory.js" --hours 72 --summary
```
