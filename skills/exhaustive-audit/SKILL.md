...
```bash
# Fleet shared cold store (not inside product git). See ~/revealfleet/archive/cold/README.md
...
All under `$HOME/revealfleet/revskills/skills/exhaustive-audit/scripts/`:
...
```bash
SKILL="$HOME/revealfleet/revskills/skills/exhaustive-audit"
export REVFLEET_ARCHIVE="${REVFLEET_ARCHIVE:-$HOME/revealfleet/archive/cold}"
node "$SKILL/scripts/open-run.js" --root "$HOME/revealfleet" --fleet --slug fleet-p0
# or one repo:
node "$SKILL/scripts/open-run.js" --root "$HOME/revealfleet/revealui" --slug revealui
...
# operator:
# REVFLEET_ARCHIVE=~/revealfleet/archive/cold node …/md-truth-check.js --coverage
```
