...
- **RevealUI Studio / RevealFleet:** secrets live in **revvault** (`revvault run --env KEY=path -- cmd` / `with-secrets`). Env vars are a downstream injection surface, not the store. See fleet `secrets` hardline. Lookup and inject: **revvault-resolve**.
